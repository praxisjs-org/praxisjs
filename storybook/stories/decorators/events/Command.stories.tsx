...
import { Component, State, Prop, OnCommand, Command } from "@praxisjs/decorators";
  ...
  render() {
    return (
      <div style="display:flex;flex-direction:column;gap:14px;font-family:sans-serif;min-width:300px">
        <h3 style="margin:0;font-size:1rem">@Command() — field decorator syntax</h3>
        ...
        <p style="margin:0;font-size:.78rem;color:#aaa">
          <code>@Command()</code> is the field-decorator form of <code>createCommand()</code> — each field gets
          its own <code>Command</code> instance, created once per component. Pass it down as a prop and consume
          it with <code>@OnCommand()</code>, exactly as with a manually-created command.
        </p>
      </div>
    );
  }
...
const meta: Meta = {
  title: "Decorators/Events/Command",
  tags: ["autodocs"],
};
export default meta;
...
export const CommandStory: Story = {
  name: "@Command() — field decorator syntax",
  render: () => <CommandDemo />,
};
