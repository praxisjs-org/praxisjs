...
**Breaking:** the `add` command has moved to a new `praxisjs` CLI package — run `npx praxisjs add` instead of `npx create-praxisjs add`. `create-praxisjs` now only scaffolds new projects; it depends on `praxisjs` to share the AI-integration plugin logic used during the "Add an AI integration?" scaffolding step.
