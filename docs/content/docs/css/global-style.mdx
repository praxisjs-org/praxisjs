...
Cascade layer priority follows the order layers first appear in the document — whichever layer is declared first ranks lowest. Call `globalStyle()` (or [`preflight()`](#preflight)) before importing CSS from another layered system so your layers rank below it.
