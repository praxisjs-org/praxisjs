...
Pass `layer` to wrap the CSS in a named `@layer`, so it can be ordered against other layered CSS (like Tailwind's `@layer utilities`) instead of always winning the cascade regardless of specificity:

```ts
globalStyle(css =>
  css({}).on('body', { fontFamily: 'system-ui', lineHeight: 1.5 }),
  { layer: 'base' }
)
```

Cascade layer priority follows the order layers first appear in the document — whichever layer is declared first ranks lowest. `globalStyle()` registers its layer at runtime via `document.head.appendChild`, so calling it before importing CSS from another layered system only reliably ranks below that system in dev mode, where both are JS-mediated. A production build often extracts the other system's CSS into a static `<link>` parsed before any JS runs, which can flip the order. See [Coexisting with Tailwind](#coexisting-with-tailwind-or-other-layered-css) for the robust fix — declaring the layer name up front in your own static CSS.
...
function globalStyle(factory: GlobalStyleFactory, options?: GlobalStyleOptions): void
```

| Parameter | Type | Description |
|---|---|---|
| `factory` | `GlobalStyleFactory` | Receives `css` (= `createCSSBuilder`) and returns a `CSSBuilder` or raw CSS string. |
| `options.layer` | `string \| false` | Wraps the CSS in a named `@layer`. Omit or pass `false` to inject un-layered CSS (default). |
...
Cascade layer priority follows the order layers first appear in the document: whichever layer is declared first ranks lowest. In **dev mode**, both `preflight()` and a JS-imported Tailwind stylesheet register their layers imperatively, via `<style>` tags injected in import order — so calling `preflight()` before importing Tailwind's CSS is enough to rank `reset` below it.

<Callout type="warn">
  This breaks in a **production build**. Tailwind's CSS is typically extracted into a static `<link rel="stylesheet">` that sits in the compiled HTML `<head>` and is parsed before any JS runs — while `preflight()` still registers its `@layer reset {...}` block at runtime, via `document.head.appendChild`. That flips the effective order: Tailwind's layers (`theme, base, components, utilities`) get registered first, and `reset` — introduced for the first time only once your JS runs — is appended *after* them, ranking **above** `utilities` and silently wiping out every Tailwind utility class. Import order alone is not reliable across dev and prod — this exact failure mode has shown up as buttons rendering as unstyled plain text in a production Storybook build that worked fine in dev.
</Callout>

The robust fix is to declare the layer order **up front, in your own static CSS**, before Tailwind's `@import` runs:

```css
/* e.g. .storybook/tailwind-theme.css */
@layer reset;
...
A bare `@layer reset;` statement (no block) doesn't inject any rules — it just registers the name. Because it's the first layer declaration in this file, `reset` is pinned as the lowest-priority layer before Tailwind's own `@layer theme, base, components, utilities;` (emitted by the `@import` below it) gets a chance to register first. This is order-independent of whichever side — Tailwind's static `<link>` or `preflight()`'s runtime `<style>` — finishes supplying its actual rules later: `reset` is already locked in below `utilities` either way.

Pass a string to `layer` to match the name you declared, or `layer: false` to opt back into un-layered CSS:

```ts
preflight({ layer: 'reset' })   // matches `@layer reset;` above (also the default)
