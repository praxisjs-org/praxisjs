...
Run the `ai add` command at the project root:
...
Run the `ai remove` command at the project root and pick the integration to uninstall:
...
This deletes the skill directory and, for Claude Code, `.claude/settings.json`. It leaves `CLAUDE.md`/`AGENTS.md` and `.praxisjs-ai.json` alone — those hold project context and preferences that aren't owned exclusively by the integration.
...
```jsonc
{
  ...
  "css": "tailwind"       // praxisjs-css | plain | modules | tailwind | unocss | none
}
```
...
## Stack
- PraxisJS 2.x + @praxisjs/core, @praxisjs/decorators, @praxisjs/runtime, @praxisjs/jsx
...
The skill itself tells you to run `praxisjs doctor` when something seems misconfigured, and `praxisjs upgrade` to update `@praxisjs/*` dependencies. Full reference for both, plus `ai add` / `ai remove`, in the [praxisjs CLI docs](/docs/tooling/cli).
