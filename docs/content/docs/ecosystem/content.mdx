...
<Callout type="warn" title="contentPlugin() must come before praxisjs()">
  Both plugins run with `enforce: "pre"`, and Vite preserves relative order within that tier — so whichever one appears first in the array runs first. `praxisjs()` includes decorator lowering, which strips `@Collection('./path/*.md')` down to a plain `Collection('./path/*.md')` call. If that runs before `contentPlugin()`'s regex transform sees the `@`, the glob string reaches `@praxisjs/content` unexpanded — `getCollection()` then iterates the string's individual characters as if they were file paths, producing one broken "entry" per character instead of your actual files.
</Callout>
...
## Static paths for `@praxisjs/ssg`

`collectionStaticPaths(Schema)` builds a per-route `getStaticPaths` for `@praxisjs/ssg` directly from a collection — every entry's `slug` is substituted into the route's dynamic segment, so there's no manual `getCollection` + `.map()` glue to write:
...
The route's own full path (`/blog/:slug`) is passed in automatically by `@praxisjs/ssg` — it must contain exactly one dynamic segment. Routes with zero or multiple params aren't a single-field substitution; write a plain `getStaticPaths` function for those instead. See the [SSG guide](/docs/tooling/ssg#dynamic-routes) for the full picture.
