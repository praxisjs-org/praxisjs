---
title: Store
description: "@praxisjs/store — class-based singleton stores with @Store and @UseStore. Define reactive state with @State, inject anywhere with @UseStore or useStore()."
icon: Store
---
...
Resolves the singleton store instance imperatively — no class field or decorator required. Use this anywhere `@UseStore` is unavailable: route guards, plain functions, or services.
...
`useStore()` and `@UseStore` share the same registry — they always return the same instance.
