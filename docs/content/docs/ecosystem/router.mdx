...
|---|---|---|
| `location` | `Signal<RouteLocation>` | Current route — `path`, `params`, `query`, `hash` |
...
| `params` | `Computed<RouteParams>` | Shorthand for `location().params` |
| `query` | `Computed<RouteQuery>` | Shorthand for `location().query` |
...
import type { RouteComponent } from '@praxisjs/router'

@Component()
class DebugBar extends StatefulComponent {
  @InjectLayout() layout!: Signal<RouteComponent | null>
