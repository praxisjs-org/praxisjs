...
description: "@praxisjs/di — decorator-based DI with @Injectable, @Inject, @InjectContainer, and @Scope for per-instance scoped containers. Use inject() for imperative resolution outside components."
icon: Plug
---
...
Resolves a dependency from the global container imperatively — no class instance required. Use this anywhere decorators are unavailable: route guards, plain functions, module initializers.
...
`inject()` always resolves from the global container. If you need scoped resolution, use `container.createChild()` and call `.resolve()` directly.
