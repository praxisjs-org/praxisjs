...
### Alternative — `@Computed({ set })`

Pass a `set` function to the decorator factory as a compact alternative. The behavior is identical at runtime; the setter's `this` is bound to the component instance.
...
<Callout type="warn" title="TypeScript readonly limitation">
  When using `@Computed({ set })`, TypeScript still sees the property as read-only because the setter is installed at runtime via `Object.defineProperty`. Assigning to the property inside the component will require a cast:

  ```ts
  (this as { fahrenheit: number }).fahrenheit = value
  ```

  Prefer the plain getter + setter pair above when TypeScript compatibility matters.
</Callout>
