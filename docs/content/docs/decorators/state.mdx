---
title: State & Props
description: Decorators for managing reactive state and external props — @State, @Prop, @FunctionProp, @Computed, @Persisted, @History, @Resource, @Synced, and @DeepState.
icon: Database
---
...
Declares an external prop whose value is a function. Use it when the parent passes a callback, predicate, formatter, or filtering function and the component should receive that function itself.

`@Prop()` treats a function prop as a reactive getter and calls it when the field is read. `@FunctionProp()` keeps the function intact.

<Callout type="warn" title="Non-function values">
  If the parent passes a non-function value, `@FunctionProp()` still returns it as-is but logs a `console.warn` in development (skipped when `NODE_ENV === "production"`).
</Callout>
...
<StorybookLink story="decorators-state-props-functionprop--function-filter" label="Live demo — @FunctionProp" />
...
<StorybookLink story="decorators-state-props-writablecomputed--temperature-converter-story" label="Live demo — @Computed with setter" />
...
<Callout type="info" title="accessor vs getter + setter">
...
<StorybookLink story="decorators-state-props-writablecomputedaccessor--temperature-converter-accessor-story" label="Live demo — @Computed({ get, set }) accessor form" />
