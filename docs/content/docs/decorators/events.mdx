...
### `@Command<T>()` — field decorator syntax

`Command<T>` is both the decorator function and the type annotation — one import, used in both positions, same as [`@Ref<T>()`](/docs/decorators/state#reft). Use it on the parent that owns the commands instead of calling `createCommand()` by hand:
...
class Player extends StatefulComponent {
  ...
  handleSeekClick(time: number) {
    this.seek.trigger(time)
  }
}
```

Each decorated field gets its own `Command` instance, created once per component instance — equivalent to writing `play = createCommand()` as a plain field initializer, but consistent with the rest of the field-decorator API.

<StorybookLink story="decorators-events-command--command-story" label="Live demo — @Command()" />
