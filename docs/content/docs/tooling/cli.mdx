...
| **`tsconfig.json`** | Missing, malformed, or `compilerOptions` doesn't have `jsx: "react-jsx"`, `jsxImportSource: "@praxisjs/jsx"`, and `useDefineForClassFields: true` — the tsconfig check only runs once `package.json` passes |
